...
if (version_compare(PHP_VERSION, '5.1.0', '>=')) {//PHP5.1.0以上の場合のみタイムゾーンを定義
	date_default_timezone_set('Asia/Tokyo');//タイムゾーンの設定（日本以外の場合には適宜設定ください http://php.net/manual/ja/timezones.php）
}
/*-------------------------------------------------------------------------------------------------------------------
* ★以下設定時の注意点　
...
$Referer_check_domain = "tipsandtips.xsrv.jp";
...
$thanksPage = "https://tipsandtips.xsrv.jp/Eatwell/";
...
//指定しない、または存在しない場合は、○○様と表示されないだけです。あえて無効にしてもOKです（特に日本人以外を想定してる場合は無効として下さい）
$dsp_name = 'お名前';
...
URL: https://tipsandtips.xsrv.jp/Eatwell
--------------------------------------------------
...
//各文言のテキストを指定（ご使用の言語に合わせて設定下さい）「'」で囲まれた日本語部分のみ変更可能です。

//必須項目が未入力だった場合に表示するメッセージ
$requireCheckMesse = '入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。';

//確認画面のメッセージ
$confirmMesse = '以下の内容で間違いがなければ、「送信する」ボタンを押してください。';

//確認画面の送信ボタンのテキスト
$submitBtnMesse = '送信する';

//戻るボタンのテキスト
$returnBtnMesse = '前画面に戻る';

//確認画面のタイトル（見出しとタイトルタグ）
$confirmTitle = '確認画面';

//メールアドレスの形式が違う場合に表示するメッセージ（このテキスト前に該当項目名が入ります）
$mailCheckMesse = 'はメールアドレスの形式が正しくありません。';

//必須入力項目が未入力の場合のメッセージ（このテキスト前に該当項目名が入ります）
$requireError['text'] = 'は必須項目です。';

//チェックボックス、ラジオボタン未入力の場合のメッセージ（このテキスト前に該当項目名が入ります）
$requireError['select'] = 'が未選択です。';



/*

その他ユーザー宛のメール本文は下記「//ユーザ宛送信メールボディ」箇所、
管理者宛のメール本文は「//管理者宛送信メールボディ」の箇所の日本語部分を必要に応じて変更下さい。（「送信日時」など）
基本的にはこのファイル内の日本語部分は変更可能です。送信テストを行っていただき必要に応じて文言は自由に変更下さい。

*/


//name属性の値のアンダーバーを半角スペースに変換する(する=1, しない=0)
//name属性の値に半角スペースは使用できないため、一時的にアンダーバーで指定しておき表示時に半角スペースに変換します。
$exchangeSpace = 0;
//----------------------------------------------------------------------
//  多言語対応版独自設定(END)
//----------------------------------------------------------------------
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $Email && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
				$errm .= "<p class=\"error_messe\">【".underline2space($key)."】".$mailCheckMesse."</p>\n";
				$empty_flag = 1;
			}
		}
	}
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
	...
	if($remail == 1) {
		if (ini_get('safe_mode')) {
		  $result = mb_send_mail($post_mail,$re_subject,$userBody,$reheader);
		} else {
		  $result = mb_send_mail($post_mail,$re_subject,$userBody,$reheader,'-f'.$from);
		}
	}
}
...
<?php if($empty_flag == 1){ ?>
<div align="center">
<h4><?php echo $requireCheckMesse;?></h4>
<?php echo $errm; ?><br /><br /><input type="button" value=" <?php echo $returnBtnMesse;?> " onClick="history.back()">
</div>
<?php }else{ ?>
<h3><?php echo $confirmTitle;?></h3>
<p align="center"><?php echo $confirmMesse;?></p>
...
<input type="submit" value="<?php echo $submitBtnMesse;?>">
<input type="button" value="<?php echo $returnBtnMesse;?>" onClick="history.back()"></p>
</form>
<?php copyright();} ?>
</div><!-- /formWrap -->
<!-- ▲ *********** 送信内容確認部　※編集は自己責任で ************ ▲-->
...
<br /><br /><input type="button" value=" <?php echo $returnBtnMesse;?> " onClick="history.back()">
</div>
</body>
</html>
<?php }else{ ?>
送信ありがとうございました。<br />
Thank you for sending email address.<br />
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div align="center"><h4><?php echo $requireCheckMesse;?></h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input type="button" value=" <?php echo $returnBtnMesse;?> " onClick="history.back()"></div>
<?php
...
function postToMail($arr){
	...
	foreach($arr as $key => $val) {
		...
		if($out != "confirm_submit" && $key != "httpReferer") {
			$resArray .= "[ ".underline2space($key)." ] ".$out."\n";
		}
	}
	return $resArray;
}
...
function confirmOutput($arr){
	...
	foreach($arr as $key => $val) {
		...
		$html .= "<tr><th>".underline2space($key)."</th><td>".$out;
...
function mailToUser($arr,$dsp_name,$remail_text,$mailFooterDsp,$mailSignature,$encode){
	...
	$userBody.="\n======================================================\n\n";
	$userBody.="Send Date：".date( "Y/m/d (D) H:i:s", time() )."\n";
...
function requireCheck($require){
	...
	foreach($require as $requireVal){
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<p class=\"error_messe\">【".underline2space(h($key))."】".$requireError['text']."</p>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
		...
		if($existsFalg != 1){
				$res['errm'] .= "<p class=\"error_messe\">【".underline2space($requireVal)."】".$requireError['select']."</p>\n";
				$res['empty_flag'] = 1;
		}
	}
...
function copyright(){
	echo '<a style="display:block;text-align:center;margin:15px 0;font-size:11px;color:#aaa;text-decoration:none" href="https://tipsandtips.xsrv.jp/" target="_blank">- Yukyco - </a>';
}

//name属性に半角スペースを使用したい場合で一時的にアンダーバーにしたものを半角スペースに変換する（英語など用）2016/8/28追加
function underline2space($str){
	global $exchangeSpace;
	if($exchangeSpace == 1){
